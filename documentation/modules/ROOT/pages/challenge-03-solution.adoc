...
- Enable Read and Write permissions to: Administration, Commit statutes, contents, dependabot alerts, deployments, pull requests, webhooks,
--
By Operator::
+
--
...
Additionally, we need to create a Personal Access Token (aka PAT) to enable the full integration with GitHub.
Create a PAT of GitHub user profile as:

- Name: `pat-rhdh-adventure`
- Expiration date: Disabled it or just one in the future
- Set the scopes:
    * Reading software components: `repo`
    * Reading organization data: `read:org`, `read:user`, `user:email`
    * Publishing software templates: `repo`
    * `workflow` (if templates include GitHub workflows)

From the new application and PAT create a secret to store the different id, secrets, and tokens as:
...
Modify `rhdh-app-config` section of the `rhdh-app-config` ConfigMap with environment variables from the new secret:
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rhdh-app-config-cm-github-01.yaml
----
...
The GitHub integration has a special entity provider for discovering catalog entities from GitHub. The entity provider
will crawl the GitHub instance and register entities matching the configured paths. This can be useful as an alternative
...
Once we have integrated GitHub with Red Hat Developer Hub, we need to enable the autodiscovery
capabilities of this provider. Very useful to load our catalog with repositories already created in our GitHub sever.
...
That variable can be declared in the `git-secrets`, as the rest of GitHub configuration variables.

The Red Hat Developer Hub catalog can be set up to ingest organizational data -- users and groups -- directly from GitHub.
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rhdh-app-config-cm-github-02.yaml
----

Finally, we need to enable some extra dynamic plugins to setup finally the integration with GitHub:
...
[source, yaml, subs="+macros,+attributes"]
----
include::example$challenge-03/rhdh-dynamic-plugins-cm-gitub-01.yaml[]
----
...
- name: `rhdh-adventure`
- Redirect URI: copy output `echo https://rhdh-developer-hub-$(oc project -q).${basedomain}/api/auth/gitlab/handler/frame`
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rhdh-app-config-cm-gitlab-01.yaml
----
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rhdh-app-config-cm-gitlab-02.yaml
----
...
Create a new `rbac-policy.csv` RBAC rules file in a new ConfigMap to declare the authorization policies:

[.console-input]
[source, yaml, subs="+macros,+attributes"]
----
include::example$challenge-03/rbac-policy-configmap-gitlab.yaml[]
----
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rbac-policy-configmap-gitlab.yaml
----
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rhdh-app-config-cm-github-03.yaml
----
--
GitLab::
+
...
[source, bash, subs="+macros,+attributes"]
----
oc apply -f ./documentation/modules/ROOT/examples/challenge-03/rhdh-app-config-cm-gitlab-03.yaml
----
--
====
...
upstream:
  backstage:
    extraAppConfig:
      - configMapRef: rhdh-app-config
        filename: rhdh-app-config.yaml
      - configMapRef: rbac-policy
        filename: rbac-policy.csv
----
...
Test and verify with different users the privileges created
...
TIP: If you login with the `root` user, you will be able to edit the RBAC policies from the `Administration` page.
