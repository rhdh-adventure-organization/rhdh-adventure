...
- **Permissions**:
    ...
    * **Organization permissions**: Enable `Read-Only` to `members` (Read-Only)
...
[source, bash, subs="+macros,+attributes"]
----
oc create configmap rbac-policies --from-file=./documentation/modules/ROOT/examples/challenge-03/rbac-policies.csv
----
...
TIP: Declare one user of your organization as the default administrator. Replace `<admin-id>` with a valid value.
